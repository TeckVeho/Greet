...
export function sortRestaurants<T extends SortableRestaurant>(
	...
	switch (sort) {
		...
		case 'price_asc':
			return arr.sort((a, b) => parsePriceLow(a.priceRange) - parsePriceLow(b.priceRange))
		case 'price_desc':
			return arr.sort((a, b) => parsePriceLow(b.priceRange) - parsePriceLow(a.priceRange))
		case 'reviews_desc':
			return arr.sort(
				(a, b) =>
					(b.reviewCount ?? b.reviews?.length ?? 0) - (a.reviewCount ?? a.reviews?.length ?? 0),
			)
		case 'rating_desc':
...
export const onError = (errors: FieldErrors<{}>) => {
	toast.error(errors && Object.values(errors)[0]?.message)
}
